...
const log = (content) => {
  const message = typeof content === "string" ? chalk__default.white(content) : content.map((ele) => chalk__default[ele.color](ele.text)).join("");
  console.log(message);
};
...
const copyFiles = ({
  ...
  return Promise.all(
    files.map((file) => {
      const dest = path__default.resolve(destDir, `.${path__default.basename(file)}`);
      return fs__default.copyFile(file, dest);
    })
  );
};
const createConfigFile = async () => {
  const json = await fs__default.readFile("package.json", "utf-8");
  const { type } = JSON.parse(json);
  const source = path__default.resolve(
    process.argv[1],
    `../../dist/tpl/${!type || type === "commonjs" ? "cjs" : "es"}`
  );
  const files = await glob__default("**/*", {
    cwd: source,
    absolute: true,
    onlyFiles: true
  });
...
const addScripts = async () => {
  ...
  try {
    ...
    await fs__default.writeFile("package.json", modifiedJson, "utf8");
    log(logs$2[1]);
    result = {
      successful: 1,
      fail: 0,
...
util__default.promisify(child_process.exec);
const logs$1 = [
  [
    {
      color: "blue",
...
const createFiles = async (dir = ".husky") => {
  ...
  if (dir.includes(".."))
    return ".. not allowed";
  fs__default.mkdirSync(path__default.join(dir, "_"), { recursive: true });
  files.forEach(
    (file) => fs__default.writeFileSync(
      path__default.join(dir, "_", file),
      `#!/usr/bin/env sh
. "\${0%/*}/h"`,
      { mode: 493 }
    )
  );
  fs__default.writeFileSync(path__default.join(dir, "_", ".gitignore"), "*");
  fs__default.writeFileSync(path__default.join(dir, "_", "husky.sh"), "");
  fs__default.writeFileSync(
    path__default.join(dir, "commit-msg"),
    `#!/usr/bin/env sh
...
npx --no -- commitlint --edit $1`,
  ...
  fs__default.writeFileSync(
    path__default.join(dir, "pre-commit"),
    `#!/usr/bin/env sh
. "$(dirname -- "$0")/_/husky.sh"
...
npm run lint:lint-staged`,
  ...
  await fs__default.copyFile(
    path__default.resolve(process.argv[1], "../../dist/tpl/husky/h"),
    path__default.join(dir, "_", "h")
  );
  return "";
};
...
const checkStat = (path) => {
  return new Promise((resolve) => {
    fs__default.stat(path, (error, stats) => {
      if (error) {
        resolve(false);
      } else {
        resolve(stats);
...
const checkCacheLog = async () => {
  ...
  if (dirStat) {
    const data = await fs__default.readFile(".tttiga-lint/cache.log", "utf-8");
    if (data.length > 0) {
      return false;
    }
    return;
...
const createCacheLog = async (taskRecordData, cacheLogStat) => {
  if (cacheLogStat) {
    await fs__default.mkdir(".tttiga-lint");
  }
  let logText = taskRecordData.logs.join("\n");
  await fs__default.writeFile(".tttiga-lint/cache.log", logText);
...
${logs[3]}
  `;
  await fs__default.writeFile(".tttiga-lint/cache.log", logText, "utf8");
  log(logs[1]);
  log(logs[2]);
  log(logs[3]);
};
...
const generate = async () => {
  ...
  if (cacheLogStat !== false) {
    ...
    log([
      ...
      {
        color: "blue",
        text: `${path__default.join(process.cwd(), "tttiga-lint/cache.log")}`
      }
    ]);
  }
};
